...
names(import)
data<-import %>% select(contains("maj_broad"), contains("maj_focal"), HyperDiploidy, matches("SeqWGS"))
names(data)
...
data %>% summarise_all(sum, na.rm =T) %>% as.numeric %>% sort %>% plot + abline(h = 25) + abline(h = 40, col="red")
rel <- data %>% summarise_all(sum, na.rm =T) %>% `>`(25) %>% as.vector
...
ggsave("C:/Users/mm_gr/Alma Mater Studiorum Università di Bologna/PROJECT 1q & 13 - Documenti/PAPER_FIGURES_OFFICIAL/fisher matrix/5.1_fisher_matrix_commpass.pdf", 
