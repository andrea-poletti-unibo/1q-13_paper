...
# setwd("D:/analisi_in_corso/risk_1q_13/CoMMpass/")
# import <- fread("complete_database_1q_13_CoMMpass.txt")

import<-fread("C:/Users/andre/Alma Mater Studiorum Università di Bologna/PROJECT 1q & 13 - Documenti/complete_database_1q_13_CoMMpass_240121.txt")


# select only relevant variables to associate
names(import)

data<-import %>% select(Study_Visit_iD , contains("maj_broad"), contains("maj_focal"), 
                        HyperDiploidy, 
...
# 
# import %>% select(missing_T, t_IgH, PROTOCOL) %>% View
...
save.image("Script_analysis_CoMMpass/6_NMDS_env.Rdata")
load("Script_analysis_CoMMpass/6_NMDS_env.Rdata")
...
outdir <- "C:/Users/mm_gr/Alma Mater Studiorum Università di Bologna/PROJECT 1q & 13 - Documenti/PAPER_FIGURES_OFFICIAL/"
...
snapshot3d(paste0(outdir,"NMDS_commpass_hyperdiploid.png"), width=10000, height=10000)


pca3d(fit3$points, col= category_t_IgH, group= as.factor(category_t_IgH), 
      show.centroids = T , axe.titles=c("NMDS1","NMDS2","NMDS3"), shape="s" )
snapshot3d(paste0(outdir,"NMDS_commpass_t_IGH.png"), width=10000, height=10000)


pca3d(fit3$points, col= category_MMrisk, group= as.factor(category_MMrisk), 
      show.centroids = T , axe.titles=c("NMDS1","NMDS2","NMDS3"),  shape="s" )
snapshot3d(paste0(outdir,"NMDS_commpass_MMgroups.png"), width=10000, height=10000)


pca3d(fit3$points, col= category_AMP_1q, group= as.factor(category_AMP_1q), 
      show.centroids = T , axe.titles=c("NMDS1","NMDS2","NMDS3"),  shape="s" )
snapshot3d(paste0(outdir,"NMDS_commpass_amp1q.png"), width=10000, height=10000)


pca3d(fit3$points, col= category_DEL_13q, group= as.factor(category_DEL_13q), 
      show.centroids = T , axe.titles=c("NMDS1","NMDS2","NMDS3"),  shape="s" )
snapshot3d(paste0(outdir,"NMDS_commpass_del13.png"), width=10000, height=10000)
...
category <- ifelse(df.2$AMP_broad_chr_1q == 1 ,"green","gray10")
pca3d(fit3$points, col= category, group= as.factor(category), show.centroids = T , axe.titles=c("NMDS1","NMDS2","NMDS3"), shape=shape )

category <- ifelse(df.2$DEL_broad_chr_13q == 1 ,"red","gray10")
pca3d(fit3$points, col= category, group= as.factor(category), show.centroids = T , axe.titles=c("NMDS1","NMDS2","NMDS3"), shape=shape )


risk_1q_13<- df.2$AMP_broad_chr_1q + df.2$DEL_broad_chr_13q
...
for(i in seq_along(CATS)){
  ...
  snapshot3d(paste0("C:/Users/andre/Alma Mater Studiorum Università di Bologna/PROJECT 1q & 13 - Documenti/PAPER_FIGURES_OFFICIAL/NMDS/",name,"_NMDS_CoMMpass_def3D.png"), height = 2000, width = 2000)
...
snapshot3d(paste0("C:/Users/andre/Alma Mater Studiorum Università di Bologna/PROJECT 1q & 13 - Documenti/PAPER_FIGURES_OFFICIAL/NMDS/",name,"_NMDS_CoMMpass_CLUSTERS.png"), height = 2000, width = 2000)
