...
names(import)
data<-import %>% select(contains("maj_broad"), contains("maj_focal"), matches("SeqWGS"))
names(data)
...
write_tsv(resultMM, "AncestralityIndex_table_CoMMpass_0600821.txt")
...
ggplot(resultF.MM, aes(x = TGC, group= as.factor(alteration))) +
  ...
  facet_wrap(alteration~., ncol = 3, nrow = 28)+
  xlim(c(0,35))

ggsave(filename = "C:/Users/andre/Alma Mater Studiorum Università di Bologna/PROJECT 1q & 13 - Documenti/Supplementary/Ancestrality_index_commpass_all_events_060821.png", 
       device = "png", width = 12, height = 36)
