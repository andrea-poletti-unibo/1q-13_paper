...
# import <- data.table::fread("./git_mmrisk/clinical_data_COMMPASS-IA13_210122.txt") 
import <- data.table::fread("clinical_data_COMMPASS-IA13_210122.txt") 
...
write_tsv(data.frame("Univariate"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
outpath <- "./git_mmrisk/Script_analysis_CoMMpass/Clinical/analysis_260122/"
write_tsv(data.frame("Univariate"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
...
# INDEX:                                                                #
#                                                                       #
#1._____ADDITIONAL DATA MANAGEMENT____(not needed, already created)____ #
#2._____SURV ANALYSIS                                                   #
#3a.____UNIVARIATE____PT1__(CLINICAL)                                   #
#3b.____UNIVARIATE___PT2_______(GENOMIC)_                               #
#4._____RISK_&_ULTRA__________                                          #
#5._____OLD MULTIVARIATE___________________                             #
#6._____MULTIVARIATE REVIDES AK_______                                  #
#########################################################################
...
import$D_IM_LIGHT_CHAIN_BY_FLOW2<- tolower(import$D_IM_LIGHT_CHAIN_BY_FLOW)
import$D_IM_LIGHT_CHAIN_BY_FLOW2<-ifelse(import$D_IM_LIGHT_CHAIN_BY_FLOW2=="","none",import$D_IM_LIGHT_CHAIN_BY_FLOW2)
import$D_IM_LIGHT_CHAIN_BY_FLOW2<-as.factor(import$D_IM_LIGHT_CHAIN_BY_FLOW2)
class(import$D_IM_LIGHT_CHAIN_BY_FLOW2)
levels(import$D_IM_LIGHT_CHAIN_BY_FLOW2)
import$D_IM_LIGHT_CHAIN_BY_FLOW2<-relevel(import$D_IM_LIGHT_CHAIN_BY_FLOW2, ref = 4)

import$NOTE_TRANSLOCATION<-ifelse(import$SeqWGS_WHSC1_CALL==1, "t(4;14)",
                                  ifelse(import$SeqWGS_CCND1_CALL==1, "t(11;14)",
                                         ifelse(import$SeqWGS_CCND3_CALL==1,"t(6;14)",
                                                ifelse(import$SeqWGS_MAF_CALL==1,"t(14;16)",
                                                       ifelse(import$SeqWGS_MAFB_CALL==1,"t(14;20)",0)))))
...
import$MMrisk_CLASSIFIER_ALL<-relevel(import$MMrisk_CLASSIFIER_ALL, ref=3)
...
table(import$SeqWGS_WHSC1_CALL)

gmodels::CrossTable(import$MMrisk_CLASSIFIER_ALL, import$SeqWGS_WHSC1_CALL)


import$MMrisk1_pure <- ifelse(import$MMrisk_CLASSIFIER_ALL==1 & import$SeqWGS_WHSC1_CALL ==0 & import$SeqWGS_MAF_CALL ==0 & import$SeqWGS_MAFB_CALL ==0, 1, 0)
import$MMrisk1_pure %>% table

import$T_4_14_pure <- ifelse(import$MMrisk_CLASSIFIER_ALL != 1 & import$SeqWGS_WHSC1_CALL ==1, 1, 0)
import$T_4_14_pure %>% table

import$T_14_16_pure <- ifelse(import$MMrisk_CLASSIFIER_ALL != 1 & import$SeqWGS_MAF_CALL ==1, 1, 0)
import$T_14_16_pure %>% table

import$T_14_20_pure <- ifelse(import$MMrisk_CLASSIFIER_ALL != 1 & import$SeqWGS_MAFB_CALL ==1, 1, 0)
import$T_14_20_pure %>% table

import$MMrisk1_and_CCND2_t <- ifelse(import$MMrisk_CLASSIFIER_ALL==1 & (import$SeqWGS_WHSC1_CALL ==1 | import$SeqWGS_MAF_CALL ==1 | import$SeqWGS_MAFB_CALL ==1) , 1, 0)
import$MMrisk1_and_CCND2_t %>% table

table(import$MMrisk1_pure, import$MMrisk_CLASSIFIER_ALL)

import$ULTRA_Clincal <- ifelse(import$D_LAB_serum_beta2_microglobulin>= 5.5 & import$MMrisk_CLASSIFIER_ALL == 1, "Ultra_High", 
                         ifelse(import$D_LAB_serum_beta2_microglobulin < 3.5 & import$D_LAB_chem_albumin >= 35 & import$MMrisk_CLASSIFIER_ALL == 3, "Ultra_Low", "Other"
                                )) %>% as.factor()
import$ULTRA_Clincal %>% table

gmodels::CrossTable(import$ULTRA_Clincal, import$ULTRA_MMrisk)

table(import$ULTRA_4, import$ULTRA_MMrisk, useNA = "always")




import$ULTRA_4 <- ifelse(import$ULTRA_Clincal=="Ultra_High" & (import$SeqWGS_WHSC1_CALL ==1 | import$SeqWGS_MAF_CALL==1 | import$SeqWGS_MAFB_CALL==1 ), "Ultra-ultra_High", import$ULTRA_Clincal %>% as.character() ) %>% as.factor()
import$ULTRA_4 %>% table

levels(import$ULTRA_4)
...
#2._____SURV ANALYSIS______________
###################################
...
#3a._____UNIVARIATE____PT1__(CLINICAL)
####################################
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ OLD  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ OLD  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ SEX )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ SEX )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ D_LAB_serum_beta2_microglobulin  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ D_LAB_serum_beta2_microglobulin  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ D_LAB_chem_albumin  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ D_LAB_chem_albumin )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ HB_m_105  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ HB_m_105  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ PLT_m_150  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ PLT_m_150  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ PC_M_60  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ PC_M_60  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ D_LAB_chem_ldh  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ D_LAB_chem_ldh  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ R_ISS  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ R_ISS  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)

#______ Light-Chain __________ 

#gg <- ggsurvplot(survfit(OS ~ import$D_IM_LIGHT_CHAIN_BY_FLOW2, data = import) , pval = T, risk.table = T, xlab = "OS", surv.median.line = "hv", break.time.by = 12, legend= c(0.9,0.9), legend.title="",  tables.y.text = F, risk.table.y.text.col = TRUE, font.legend=c("bold"))
#print(gg)
#ggsave(plot = #print(gg), filename = "LIGHT_CHAIN_OS.png", path = outpath, dpi = 300, height = 6, width = 8)
#gg <- ggsurvplot(survfit(PFS ~ import$D_IM_LIGHT_CHAIN_BY_FLOW2, data = import) , pval = T, risk.table = T, xlab = "PFS", surv.median.line = "hv", break.time.by = 12, legend= c(0.9,0.9), legend.title="",  tables.y.text = F, risk.table.y.text.col = TRUE, font.legend=c("bold"))
#print(gg)
#ggsave(plot = #print(gg), filename = "LIGHT_CHAIN_PFS.png", path = outpath, dpi = 300, height = 6, width = 8)


write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ D_IM_LIGHT_CHAIN_BY_FLOW2  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ D_IM_LIGHT_CHAIN_BY_FLOW2  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ sctflag  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ sctflag  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
####################################



#3b.______UNIVARIATE___PT2_______(GENOMIC)_
######################################


#_______NOTE TRANSLOCATION (all together)
# t(4:14): import$SeqWGS_WHSC1_CALL
# t(11;14): import$SeqWGS_CCND1_CALL
# t(6;14):import$SeqWGS_CCND3_CALL
# t(14;16): import$SeqWGS_MAF_CALL
# t(14;20): import$SeqWGS_MAFB_CALL

table(import$NOTE_TRANSLOCATION)

with(import, coxph(OS ~ import$NOTE_TRANSLOCATION ))
...
with(import, coxph(OS ~ import$NOTE_TRANSLOCATION )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_240221.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ import$NOTE_TRANSLOCATION)) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_240221.txt"), append = T, col_names = T)

#ALL TRANSLOCATION SEPARATELY
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ SeqWGS_WHSC1_CALL  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ SeqWGS_WHSC1_CALL  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ SeqWGS_CCND1_CALL  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ SeqWGS_CCND1_CALL  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ SeqWGS_MAF_CALL  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ SeqWGS_MAF_CALL  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ SeqWGS_MAFB_CALL  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ SeqWGS_MAFB_CALL  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ SeqWGS_CCND3_CALL  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ SeqWGS_CCND3_CALL  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ Hyperdiploid  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ Hyperdiploid  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ `DEL_maj_broad_chr_17p`  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ `DEL_maj_broad_chr_17p`  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ `DEL_maj_broad_chr_1p`  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ `DEL_maj_broad_chr_1p`  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ `DEL_maj_focal_TP53`  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ `DEL_maj_focal_TP53`  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ `AMP_maj_broad_chr_1q`  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ `AMP_maj_broad_chr_1q`  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ MMrisk_1q_all  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ MMrisk_1q_all  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ `DEL_maj_broad_chr_13q`  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ `DEL_maj_broad_chr_13q`  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ MMrisk_13_all  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ MMrisk_13_all  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
######################################


#4.________RISK_&_ULTRA__________
##################################


#______ MM RISK ________ (3 CLASSES)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ MMrisk_CLASSIFIER_ALL  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ MMrisk_CLASSIFIER_ALL  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ MMrisk1  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ MMrisk1  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ MMrisk2  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ MMrisk2  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ MMrisk3  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ MMrisk3  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)


#____ MM RISK a/b _______ YES! BUONO
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ MMrisk_AB_ALL  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ MMrisk_AB_ALL  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)



#____ Risk_1 vs only_1q _______no
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ risk1_VS_1q_VS_other  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ risk1_VS_1q_VS_other  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(imp1q, coxph(OS1q ~ risk1_VS_1q_VS_other  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(imp1q, coxph(PFS1q ~ risk1_VS_1q_VS_other  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)


#______ ultra MMrisk classificator ______  yes
import$ULTRA_MMrisk<-as.factor(import$ULTRA_MMrisk)
table(import$ULTRA_MMrisk)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ ULTRA_MMrisk  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ ULTRA_MMrisk  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ ULTRA_High  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ ULTRA_High  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ ULTRA_Low  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
with(import, coxph(PFS ~ ULTRA_Low  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
##################################
...
#5.________OLD MULTIVARIATE___________________
#############################################
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
write_tsv(data.frame("Multivariate"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)

write_tsv(data.frame("MULTIVARIATA 1 - MMrisk1 - OS "),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ OLD + female_gender + D_LAB_serum_beta2_microglobulin + D_LAB_chem_albumin + HB_m_105 + PLT_m_150 + D_LAB_chem_ldh + MMrisk1 + sctflag + `DEL_maj_focal_TP53` + SeqWGS_MAFB_CALL , data = import )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ female_gender + D_LAB_serum_beta2_microglobulin + PLT_m_150 + D_LAB_chem_ldh + MMrisk1 + sctflag, data = import))  %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)



write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
write_tsv(data.frame("MULTIVARIATA 2 - MMrisk1 - PFS "),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(PFS ~ OLD + female_gender + D_LAB_serum_beta2_microglobulin + D_LAB_chem_albumin + HB_m_105 + PLT_m_150 + D_LAB_chem_ldh + MMrisk1 + sctflag, data = import )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(PFS ~  female_gender + D_LAB_serum_beta2_microglobulin + HB_m_105 + D_LAB_chem_ldh + MMrisk1 + sctflag, data = import )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)



write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
write_tsv(data.frame("MULTIVARIATA 3 - Ultra High - OS "),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ OLD + female_gender + HB_m_105 + PLT_m_150 + D_LAB_chem_ldh + ULTRA_High + sctflag + `DEL_maj_focal_TP53` + SeqWGS_MAFB_CALL , data = import  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ female_gender + PLT_m_150 + D_LAB_chem_ldh + ULTRA_High + sctflag, data = import)) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)



write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
write_tsv(data.frame("MULTIVARIATA 4 - Ultra High - PFS "),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(PFS ~ OLD + female_gender + HB_m_105 + PLT_m_150 + D_LAB_chem_ldh + ULTRA_High + sctflag, data = import  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(PFS ~ OLD + female_gender + HB_m_105 + PLT_m_150 + D_LAB_chem_ldh + ULTRA_High + sctflag, data = import )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
...
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
write_tsv(data.frame("MULTIVARIATA 5 - MMrisk3 - OS "),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ OLD + female_gender + D_LAB_serum_beta2_microglobulin + D_LAB_chem_albumin + HB_m_105 + PLT_m_150 + D_LAB_chem_ldh + MMrisk3 + sctflag + `DEL_maj_focal_TP53` + SeqWGS_MAFB_CALL , data = import  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ female_gender + D_LAB_serum_beta2_microglobulin + PLT_m_150 + D_LAB_chem_ldh + MMrisk3 + sctflag , data = import )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)



write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
write_tsv(data.frame("MULTIVARIATA 6 - MMrisk3 - PFS "),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(PFS ~ OLD + female_gender + D_LAB_serum_beta2_microglobulin + D_LAB_chem_albumin + HB_m_105 + PLT_m_150 + D_LAB_chem_ldh + MMrisk3 + sctflag, data = import )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(PFS ~ D_LAB_serum_beta2_microglobulin + HB_m_105 + D_LAB_chem_ldh + MMrisk3 + sctflag, data = import)) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)



write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
write_tsv(data.frame("MULTIVARIATA 7 - UltraLow - OS "),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ OLD + female_gender + HB_m_105 + PLT_m_150 + D_LAB_chem_ldh + ULTRA_Low + sctflag + `DEL_maj_focal_TP53` + SeqWGS_MAFB_CALL , data = import  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(OS ~ female_gender + PLT_m_150 + D_LAB_chem_ldh + ULTRA_Low + sctflag , data = import  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="OS", .) %>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)



write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
write_tsv(data.frame("MULTIVARIATA 8 - UltraLow - PFS "),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(PFS ~ OLD + female_gender + HB_m_105 + PLT_m_150 + D_LAB_chem_ldh + ULTRA_Low + sctflag, data = import  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
write_tsv(data.frame("\n"),paste0(outpath,"report_cox_analysis_260122.txt"), append = T)
with(import, coxph(PFS ~ female_gender + HB_m_105 + PLT_m_150 + D_LAB_chem_ldh + ULTRA_Low + sctflag, data = import  )) %>% tidy( exponentiate =T, conf.int = T) %>% cbind(surv="PFS", .)%>% write_tsv(paste0(outpath,"report_cox_analysis_260122.txt"), append = T, col_names = T)
#############################################


######## 26/01/22 analysis ########

#_____ group mmrisk with trasnlocations ________
# OS
mv_R <- coxph(OS ~ OLD + D_LAB_serum_beta2_microglobulin + female_gender + D_LAB_chem_albumin + HB_m_105 + PLT_m_150 + D_LAB_chem_ldh+
                MMrisk1_and_CCND2_t + MMrisk1_pure + MMrisk2 + 
                T_4_14_pure + T_14_16_pure + T_14_20_pure + SeqWGS_CCND1_CALL + SeqWGS_CCND3_CALL +
                DEL_maj_focal_TP53 + DEL_maj_broad_chr_1p + HyperDiploidy, data = import )

mv_R %>% summary()

# PFS
mv_R <- coxph(PFS ~ OLD + D_LAB_serum_beta2_microglobulin + female_gender + D_LAB_chem_albumin + HB_m_105 + PLT_m_150 + D_LAB_chem_ldh+
                MMrisk1_and_CCND2_t + MMrisk1_pure + MMrisk2 + 
                T_4_14_pure + T_14_16_pure + T_14_20_pure + SeqWGS_CCND1_CALL + SeqWGS_CCND3_CALL +
                DEL_maj_focal_TP53 + DEL_maj_broad_chr_1p + HyperDiploidy, data = import )
...
mv_R <- coxph(OS ~ OLD + D_LAB_serum_beta2_microglobulin + female_gender + D_LAB_chem_albumin + HB_m_105 + PLT_m_150 + D_LAB_chem_ldh+
                MMrisk1 + MMrisk2 +
                SeqWGS_WHSC1_CALL + SeqWGS_MAF_CALL + SeqWGS_MAFB_CALL + SeqWGS_CCND1_CALL + SeqWGS_CCND3_CALL +
                DEL_maj_focal_TP53 + DEL_maj_broad_chr_1p + HyperDiploidy, data = import )
...
# 6.______MULTIVARIATE REVIDES AK_______
#########################################

#._____RISK______(3 CAT)
mv_RISK3 <-  with(import, coxph(OS ~ OLD + female_gender + D_LAB_serum_beta2_microglobulin + D_LAB_chem_albumin + HB_m_105 + PLT_m_150 +  DEL_maj_focal_TP53 + DEL_maj_broad_chr_1p + HyperDiploidy +
                             D_LAB_chem_ldh + MMrisk1 + MMrisk2 + NOTE_TRANSLOCATION + sctflag, data = import )) 
mv_RISK3 %>% summary()


#check the assumptions of ph cox model
res.cox <- coxph(OS ~ OLD + female_gender + D_LAB_serum_beta2_microglobulin + D_LAB_chem_albumin + HB_m_105 + PLT_m_150 +
                   D_LAB_chem_ldh + MMrisk1 + MMrisk2 + NOTE_TRANSLOCATION + sctflag, data = import )
res.cox
test.ph <- cox.zph(res.cox)
ggcoxzph(test.ph)
ggcoxdiagnostics(mv_RISK3, type = , linear.predictions = TRUE)


mv_RISK3_pfs <-  with(import, coxph(PFS ~ OLD + female_gender + D_LAB_serum_beta2_microglobulin + D_LAB_chem_albumin + HB_m_105 + PLT_m_150 +
                                  D_LAB_chem_ldh + MMrisk1 + MMrisk2 +NOTE_TRANSLOCATION+ sctflag, data = import )) 
mv_RISK3_pfs %>% summary()

ggforest2(mv_RISK3) 
ggsave(paste0(outpath,"FOREST_plot_mmrisk_OS.png"), main = "OS_3cat", width = 8, height = 8)

ggforest2(mv_RISK3_pfs) 
ggsave(paste0(outpath,"FOREST_plot_mmrisk_PFS.png"), main = "PFS_3cat", width = 8, height = 8)


#._____RISK______(4 CAT)
import$MMrisk_AB_ALL # contiene i 4 gruppi


#_create variabili dicotomiche per far si che funzioni il funnel plot:
#_non funziona comunque
# import$AB_none<-as.factor(ifelse(import$MMrisk_AB_ALL=="3",1,0))
# import$AB_13<-as.factor(ifelse(import$MMrisk_AB_ALL=="2_13",1,0))
# import$AB_1q<-as.factor(ifelse(import$MMrisk_AB_ALL=="2_1q",1,0))
# import$AB_all<-as.factor(ifelse(import$MMrisk_AB_ALL=="1",1,0))

table(import$AB_none,import$AB_13,import$AB_1q,import$AB_all)


mv_RISK4 <-  with(import, coxph(OS ~ OLD + female_gender + D_LAB_serum_beta2_microglobulin + D_LAB_chem_albumin + HB_m_105 + PLT_m_150 +
                                  D_LAB_chem_ldh + MMrisk_AB_ALL + NOTE_TRANSLOCATION + sctflag, data = import )) 
mv_RISK4 %>% summary()

mv_RISK4_pfs <-  with(import, coxph(PFS ~ OLD + female_gender + D_LAB_serum_beta2_microglobulin + D_LAB_chem_albumin + HB_m_105 + PLT_m_150 +
                                  D_LAB_chem_ldh + AB_13 + AB_1q + AB_all + NOTE_TRANSLOCATION + sctflag, data = import )) 
mv_RISK4_pfs %>% summary()

ggforest2(mv_RISK4) # non va
ggsave(paste0(outpath,"FOREST_plot_mmrisk4_OS.png"), main = "OS_4cat", width = 8, height = 8)

ggforest2(mv_RISK4_pfs) #non va
ggsave(paste0(outpath,"FOREST_plot_mmrisk4_PFS.png"), main = "PFS_4cat", width = 8, height = 8)


#._____ultra______(3 CAT)

mv_ULTRA <-  with(import, coxph(OS ~ OLD + female_gender + D_LAB_serum_beta2_microglobulin + D_LAB_chem_albumin + HB_m_105 + PLT_m_150 +
                                  D_LAB_chem_ldh + ULTRA_High + ULTRA_Low + NOTE_TRANSLOCATION + sctflag, data = import )) 
mv_ULTRA %>% summary()

mv_ULTRA_pfs <-  with(import, coxph(PFS ~ OLD + female_gender + D_LAB_serum_beta2_microglobulin + D_LAB_chem_albumin + HB_m_105 + PLT_m_150 +
                                      D_LAB_chem_ldh +ULTRA_High + ULTRA_Low + NOTE_TRANSLOCATION + sctflag, data = import )) 
mv_ULTRA_pfs %>% summary()

ggforest2(mv_ULTRA) 
ggsave(paste0(outpath,"FOREST_plot_ULTRA_OS.png"), main = "OS_ULTRA", width = 8, height = 8)

ggforest2(mv_ULTRA_pfs) 
ggsave(paste0(outpath,"FOREST_plot_ULTRA_PFS.png"), main = "PFS_ULTRA", width = 8, height = 8)
...
gg <- ggsurvplot(survfit(OS ~ import$ULTRA_Clincal, data = import) , pval = T, risk.table = T, xlab = "OS", surv.median.line = "hv", break.time.by = 12, legend= c(0.9,0.9), legend.title="", tables.y.text = F, risk.table.y.text.col = TRUE, font.legend=c("bold"))
gg


gg <- ggsurvplot(survfit(PFS ~ import$ULTRA_Clincal, data = import) , pval = T, risk.table = T, xlab = "PFS", surv.median.line = "hv", break.time.by = 12, legend= c(0.9,0.9), legend.title="", tables.y.text = F, risk.table.y.text.col = TRUE, font.legend=c("bold"))
gg



gg <- ggsurvplot(survfit(OS ~ import$ULTRA_4, data = import) , pval = T, risk.table = T, xlab = "OS", surv.median.line = "hv", break.time.by = 12, legend= c(0.9,0.9), legend.title="", tables.y.text = F, risk.table.y.text.col = TRUE, font.legend=c("bold"))
gg


gg <- ggsurvplot(survfit(PFS ~ import$ULTRA_4, data = import) , pval = T, risk.table = T, xlab = "PFS", surv.median.line = "hv", break.time.by = 12, legend= c(0.9,0.9), legend.title="", tables.y.text = F, risk.table.y.text.col = TRUE, font.legend=c("bold"))
gg


# OS
mv_R <- coxph(OS ~ OLD +  female_gender + HB_m_105 + PLT_m_150 + D_LAB_chem_ldh+
                ULTRA_Clincal +
                SeqWGS_WHSC1_CALL + SeqWGS_MAF_CALL + SeqWGS_MAFB_CALL + SeqWGS_CCND1_CALL + SeqWGS_CCND3_CALL +
                DEL_maj_focal_TP53 + DEL_maj_broad_chr_1p + HyperDiploidy, data = import )
...
# PFS
mv_R <- coxph(PFS ~ OLD +  female_gender + HB_m_105 + PLT_m_150 + D_LAB_chem_ldh +
                ULTRA_Clincal +
                SeqWGS_WHSC1_CALL + SeqWGS_MAF_CALL + SeqWGS_MAFB_CALL + SeqWGS_CCND1_CALL + SeqWGS_CCND3_CALL +
                DEL_maj_focal_TP53 + DEL_maj_broad_chr_1p + HyperDiploidy, data = import )
