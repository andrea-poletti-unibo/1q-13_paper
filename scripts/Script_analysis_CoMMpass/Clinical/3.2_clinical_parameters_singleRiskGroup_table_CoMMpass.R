...
import <- fread("C:/Users/andre/Alma Mater Studiorum Università di Bologna/PROJECT 1q & 13 - Documenti/complete_database_1q_13_CoMMpass_240121.txt", na.strings=c("","NA"))
# import <- fread("D:/analisi_in_corso/risk_1q_13/CoMMpass/complete_database_1q_13_CoMMpass_240121.txt") 


# add sex and clincal data
import_clinical <- fread("C:/Users/andre/Alma Mater Studiorum Università di Bologna/MM group - Documenti/EVERGREEN_inter-projects_files/COMMPASS/IA13a/Clinical_Flat_Files/CoMMpass_IA13_FlatFiles/MMRF_CoMMpass_IA13_PER_PATIENT.csv")
...
data$MM_risk_1 <- ifelse(MM_risk==1, "1q&13+","other")
data$MM_risk_2 <- ifelse(MM_risk==2, "1q/13","other")
data$MM_risk_3 <- ifelse(MM_risk==3, "1q&13-","other")
...
for(i in data_var_num %>% names){
  ...
  try(wilcox.pval <- wilcox.test(data_var_num[,i]~data_var_num[,groupvar])$p.value %>% round(4))
  
  try(name_means_p <- c(i, wilcox.pval, means, n, NAs))
...
xlsx::write.xlsx(res_num_df, sheetName = "risk1_NUM_vars",
                 "C:/Users/andre/Alma Mater Studiorum Università di Bologna/PROJECT 1q & 13 - Documenti/Supplementary/Tables/clinical_variables_in_MMrisk1_2_3_CoMMpass.xlsx",
                 row.names = F)
...
data_var_cat_def <- data_var_cat %>% select(
  # !starts_with("D_TRI") &
  !ends_with("DETECTED") &
    # !starts_with("HD") & 
    # !starts_with("DEL") & 
...
data_var_cat_def_dummy <- data_var_cat_def %>% dummy_cols(ignore_na = T) %>% select(-contains("PUBLIC_ID"))
...
for(i in data_var_cat_def_dummy %>% names) {
  ...
  g1_SUM <- sum(name_df[,3]) 
  g2_SUM <- sum(name_df[,4])
  
  name_df_perc <- name_df %>% mutate(!!paste0(names(name_df[3]),"_perc") := round(name_df[,3]/g1_SUM, 4), 
                                     !!paste0(names(name_df[4]),"_perc") := round(name_df[,4]/g2_SUM, 4) )
  
  try(res <- rbind(res, name_df_perc) )
...
xlsx::write.xlsx(res_cat_df, sheetName = "risk1_CAT_vars", append = T,
                 "C:/Users/andre/Alma Mater Studiorum Università di Bologna/PROJECT 1q & 13 - Documenti/Supplementary/Tables/clinical_variables_in_MMrisk1_2_3_CoMMpass.xlsx",
                 row.names = F)
...
xlsx::write.xlsx(res_num_df, sheetName = "risk2_NUM_vars", append = T,
                 "C:/Users/andre/Alma Mater Studiorum Università di Bologna/PROJECT 1q & 13 - Documenti/Supplementary/Tables/clinical_variables_in_MMrisk1_2_3_CoMMpass.xlsx",
                 row.names = F)
...
xlsx::write.xlsx(res_cat_df, sheetName = "risk2_CAT_vars", append = T,
                 "C:/Users/andre/Alma Mater Studiorum Università di Bologna/PROJECT 1q & 13 - Documenti/Supplementary/Tables/clinical_variables_in_MMrisk1_2_3_CoMMpass.xlsx",
                 row.names = F)
...
xlsx::write.xlsx(res_num_df, sheetName = "risk3_NUM_vars", append = T,
                 "C:/Users/andre/Alma Mater Studiorum Università di Bologna/PROJECT 1q & 13 - Documenti/Supplementary/Tables/clinical_variables_in_MMrisk1_2_3_CoMMpass.xlsx",
                 row.names = F)
...
data_var_cat <- data %>% select(where(function(x) length(unique(x))<10 && length(unique(x))>1), 
                                !!as.symbol(groupvar), 
                                !!as.symbol(ID))
...
xlsx::write.xlsx(res_cat_df, sheetName = "risk3_CAT_vars", append = T,
                 "C:/Users/andre/Alma Mater Studiorum Università di Bologna/PROJECT 1q & 13 - Documenti/Supplementary/Tables/clinical_variables_in_MMrisk1_2_3_CoMMpass.xlsx",
                 row.names = F)
