...
segm <- fread("D:/analisi_in_corso/risk_1q_13/IGV/IGV_Rawsegm_medianCentered_Scaled.seg") 

# visual check
segm %>% ggplot(aes(seg.mean_medAdj, weight = num.mark)) + geom_density() + xlim(-2,4) + geom_vline(xintercept = -1, colour="blue") + geom_vline(xintercept = 1, colour="red")

# import focal genes
foc_genes <- readxl::read_xlsx("D:/analisi_in_corso/risk_1q_13/focal_genes_panel_1q&13.xlsx") 
...
write.table(CALLS_AMP_MAJOR_sel,"D:/analisi_in_corso/risk_1q_13/GISTIC/FOCAL_CALLS/AMP_CALLS_major_focal.txt", sep = "\t", quote = F, row.names = T)
...
write.table(CALLS_DEL_MAJOR_sel,"D:/analisi_in_corso/risk_1q_13/GISTIC/FOCAL_CALLS/DEL_CALLS_major_focal.txt", sep = "\t", quote = F, row.names = T)
...
write.table(CALLS_AMP_CLONAL_sel, "D:/analisi_in_corso/risk_1q_13/GISTIC/FOCAL_CALLS/AMP_CALLS_clonal_focal.txt", sep = "\t", quote = F, row.names = T)
...
write.table(CALLS_DEL_CLONAL_sel,"D:/analisi_in_corso/risk_1q_13/GISTIC/FOCAL_CALLS/DEL_CALLS_clonal_focal.txt", sep = "\t", quote = F, row.names = T)
...
write.table(CALLS_AMP_ALL_sel, "D:/analisi_in_corso/risk_1q_13/GISTIC/FOCAL_CALLS/AMP_CALLS_all_focal.txt", sep = "\t", quote = F, row.names = T)
...
write.table(CALLS_DEL_ALL_sel,"D:/analisi_in_corso/risk_1q_13/GISTIC/FOCAL_CALLS/DEL_CALLS_all_focal.txt", sep = "\t", quote = F, row.names = T)
