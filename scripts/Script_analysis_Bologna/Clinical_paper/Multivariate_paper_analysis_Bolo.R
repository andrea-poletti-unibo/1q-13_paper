...
outpath <- "C:/Users/mm_gr/Alma Mater Studiorum Università di Bologna/PROJECT 1q & 13 - Documenti/PAPER_FIGURES_OFFICIAL/forest_plots/"
dir.create(outpath)
...
import %>% select(MMrisk_class_t_CCND2, FISH_T_4_14, FISH_T_14_16, FISH_T_14_20, MMrisk_CLASS) %>% View                                         
...
# mv_R %>% summary()
# 
# ggforest2(mv_R, main = "OS MM-BO (model Ultra)") 
# 
# ggsave(paste0(outpath,"FOREST_OS_Bolo_ULTRA.pdf"), width = 10, height = 7)
# 
# 
# # PFS Ultra
# mv_R <- coxph(PFS ~ OLD + SEX + 
...
# mv_R %>% summary()
# 
# ggforest2(mv_R, main = "PFS MM-BO (model Ultra)") 
# 
# ggsave(paste0(outpath,"FOREST_PFS_Bolo_ULTRA.pdf"), width = 10, height = 7)



# #________________ other random analysis___________________
# 
# #_____ MM risk in FISH ____
# 
# import$MMrisk_class_FISH <- import$FISH_Amp_1q + import$FISH_DEL_13
# 
# import$MMrisk_class_FISH %>% table
# 
# 
# gg <- ggsurvplot(survfit(OS ~ import$MMrisk_class_FISH, data = import) , pval = T, risk.table = T, xlab = "OS", surv.median.line = "hv", break.time.by = 12, legend= c(0.9,0.9), legend.title="", tables.y.text = F, risk.table.y.text.col = TRUE, font.legend=c("bold"))
# print(gg)
# gg <- ggsurvplot(survfit(PFS ~ import$MMrisk_class_FISH, data = import) , pval = T, risk.table = T, xlab = "PFS", surv.median.line = "hv", break.time.by = 12, legend= c(0.9,0.9), legend.title="", tables.y.text = F, risk.table.y.text.col = TRUE, font.legend=c("bold"))
# print(gg)
# 
# 
# 
...
# #============== model 3 with R_ISS ================
# 
# gg <- ggsurvplot(survfit(OS ~ import$CCND2_ULTRA, data = import) , pval = T, risk.table = T, xlab = "OS", surv.median.line = "hv", break.time.by = 12, legend= c(0.9,0.9), legend.title="", tables.y.text = F, risk.table.y.text.col = TRUE, font.legend=c("bold"))
# print(gg)
# gg <- ggsurvplot(survfit(PFS ~ import$CCND2_ULTRA, data = import) , pval = T, risk.table = T, xlab = "PFS", surv.median.line = "hv", break.time.by = 12, legend= c(0.9,0.9), legend.title="", tables.y.text = F, risk.table.y.text.col = TRUE, font.legend=c("bold"))
# print(gg)
# 
# # OS Ultra
# mv_R <- coxph(OS ~ OLD + SEX + 
#                 HB_m_105 + PLT_m_150 +  
#                 MMrisk_class_t_CCND2 + 
#                 FISH_Del_1p36 + HyperDiploidy +
#                 R_ISS+
#                 strata(PROTOCOL_REV), data = import )
# mv_R %>% summary()
# 
# ggforest2(mv_R, main = "OS MM-BO (model 3)") 
# 
# # ggsave(paste0(outpath,"FOREST_OS_Bolo_ULTRA.pdf"), width = 10, height = 7)
# 
# 
...
# mv_R %>% summary()
# 
# ggforest2(mv_R, main = "PFS MM-BO (model 3)") 
# 
# # ggsave(paste0(outpath,"FOREST_PFS_Bolo_ULTRA.pdf"), width = 10, height = 7)
# 
# 
...
#                 HB_m_105 + PLT_m_150 +  
#                 MMrisk_class_t_CCND2 + 
#                 FISH_Del_1p36 + HyperDiploidy + FISH_Del_17p + 
#                 ISS+
#                 FISH_T_4_14_pure + FISH_T_14_16_pure + FISH_T_14_20_pure + FISH_T_11_14 + FISH_T_6_14 +
#                 strata(PROTOCOL_REV), data = import )
# mv_R %>% summary()
# 
# ggforest2(mv_R, main = "OS MM-BO (model 4)") 
# 
# # ggsave(paste0(outpath,"FOREST_OS_Bolo_ULTRA.pdf"), width = 10, height = 7)
# 
# 
...
# mv_R %>% summary()
# 
# ggforest2(mv_R, main = "PFS MM-BO (model 4)") 
# 
# # ggsave(paste0(outpath,"FOREST_PFS_Bolo_ULTRA.pdf"), width = 10, height = 7)
...
output_bo<-output[,c(1,2,3,4,5,9)]
names(output_bo)<-c("Variable","OS/PFS", "HR","Lower CI", "Upper CI", "P-value")

sjPlot::tab_df(output_bo) 

#16/02 - da aggiungere commento con le reference alla tabella
levels(import$MMrisk_class_t_CCND2)[[1]]
# PROTOCOL_REV - AMB
# ISS - 2
# B2M_level - normal
#IgH_translocation_type - no translocation
# Induction_therapy - other
# MMrisk_class_t_CCND2 - 1q&13-
