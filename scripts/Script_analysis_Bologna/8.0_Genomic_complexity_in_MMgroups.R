...
names(import)
data<-import %>% select(sample, contains("maj_broad"), contains("maj_focal"), HyperDiploidy, matches("^t_"))
names(data)
...
df.2 %>% ggplot(aes(group, SUM)) +
  geom_violin(aes(fill=group, colour=group), alpha=0.5, outlier.shape = NA) + 
...
ggsave(filename = "C:/Users/andre/Alma Mater Studiorum Università di Bologna/PROJECT 1q & 13 - Documenti/Supplementary/Figures/genomic_complexity_in_MMriskGroups.pdf", 
