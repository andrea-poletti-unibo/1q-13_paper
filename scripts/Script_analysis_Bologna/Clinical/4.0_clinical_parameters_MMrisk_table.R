...
for(i in data_var_cat_def_dummy %>% names) {
  ...
  table(data_var_cat_def_dummy$PROTOCOL_group_EMN02, data_var_cat_def_dummy$MM_group)%>% as.data.frame.matrix() %>% as.data.frame()%>% rownames_to_column(var = "type")
  
  GM <- gmodels::CrossTable(data_var_cat_def_dummy$PROTOCOL_group_EMN02, data_var_cat_def_dummy$MM_group)
  
  GM$t %>% as.data.frame.matrix()
