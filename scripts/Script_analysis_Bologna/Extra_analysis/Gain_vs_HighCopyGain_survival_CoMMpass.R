...
armsCN_scaled <- apply(armsCN, 2, function(x) x/1 ) %>% as.data.frame()
...
clinical_comm <- data.table::fread("C:/Users/mm_gr//Alma Mater Studiorum Università di Bologna/PROJECT 1q & 13 - Documenti/complete_database_1q_13_CoMMpass_240121.txt") 

merge <- left_join(chr1q, clinical_comm, by=c("sample"="Study_Visit_iD"))
...
OS_1q <- Surv(importAMP1q$OS_months, importAMP1q$censos)
PFS_1q <- Surv(importAMP1q$PFS_months, importAMP1q$censpfs)
...
gmodels::CrossTable(import$status, import$DEL_maj_broad_chr_13q, prop.chisq = F, fisher = T)
