...
import$MMrisk_CLASSIFIER_ALL <- dplyr::recode(as.character(import$MMrisk_CLASSIFIER_ALL), "1"="risk1","2"="other","3"="other")
...
NAME<-"CoMMpass_risk1_vs_others" # set name of analysis

path <- "results/DEG_analysis/r1_vs_others/"
...
pdf("plots/DEG_analysis/r1_vs_others/limma_gene_expression_MDS.pdf", width = 10, height = 5)
...
glMDSPlot(lcpm, labels="group", groups=x$samples$group, path = path,
...
dev.off()
pdf("plots/DEG_analysis/r1_vs_others/volcano_500_DEG.pdf", width = 9, height = 7)
...
pdf("plots/DEG_analysis/r1_vs_others/volcano_500_strict-DEG.pdf", width = 9, height = 7)
...
write.fit(tfit, dt, file=paste0("results/DEG_analysis/r1_vs_others/DEG_results_",NAME,".txt")) # analysis result
write_tsv(tabStrict,paste0("results/DEG_analysis/r1_vs_others/DEG_genes_",NAME,".txt")) # strict DE list
...
glMDPlot(tfit, coef=1, status=dt, main=colnames(tfit)[1], path = path, side.main="genes.hgnc_symbol", counts=x$counts, groups=group, launch=F ) # Glimma INTERACTIVE
...
ggsave("plots/DEG_analysis/r1_vs_others/Volcano_plot_strict_DEG.pdf", device = "pdf", width = 8, height = 7)
...
import$MMrisk_CLASSIFIER_ALL <- 3  - import$MMrisk_1q_all - import$MMrisk_13_all
import$MMrisk_CLASSIFIER_ALL <- dplyr::recode(as.character(import$MMrisk_CLASSIFIER_ALL), "1"="1q&13+","2"="others","3"="others")
...
ph <- pheatmap(mat_c, 
               ...
               color=colorRampPalette(c("navy", "white", "red"))(50),
               width = 12, height = 3,
               filename = "plots/DEG_analysis/r1_vs_others/Cyclin_clusters.pdf")
dev.off()
...
anno_colors = list(
  group_1q_13 = c(`1q&13+` = "orange", `others` = "#66c2a5"),
...
pheatmap(mat_r, 
         ...
         height = 8,
         color=colorRampPalette(c("navy", "white", "red"))(50), 
         filename = "plots/DEG_analysis/r1_vs_others/annotated_genes_heatmap.pdf"
)
dev.off()
